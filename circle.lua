...
function Library:New2DCircle()
    ...
    _circle.circle = nDrawing("Circle");

    -- Update Step Function --
    function _circle:Update()
        if not _circle.Visible then
            _circle.circle.Visible = false;
        else
            _circle.circle.Visible      = _circle.Visible      or _defaults.Visible;
            _circle.circle.Filled      = _circle.Filled      or _defaults.Filled;
            _circle.circle.ZIndex       = _circle.ZIndex       or _defaults.ZIndex;
            _circle.circle.Transparency = _circle.Transparency or _defaults.Transparency;
            _circle.circle.Color        = _circle.Color        or _defaults.Color;
            _circle.circle.Thickness    = _circle.Thickness    or _defaults.Thickness;
            _circle.circle.Radius    = _circle.Radius    or _defaults.Radius;

            local _position, v1 = ToScreen(Camera, _circle.Position);

            if (v1) or checkCamView(_circle.Position) then
                _circle.circle.Position = nVector2(_position.x, _position.y);
            else
                _circle.circle.Visible = false;
            end;
        end
    ...
    local step_Id = "2D_Circle"..random_string(10);

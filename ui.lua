...
local function InitWindow(ScreenAsset,Window)
	...
	WindowAsset.Title.Font = LibraryFont
	...
	local WindowEnabled = WindowAsset.Visible
	game.UserInputService.InputBegan:Connect(function(Keybind, Verified)
		if not Verified then
			if Keybind.KeyCode == _G.SilkyConfig.Keybind then
				if WindowEnabled == true then
					WindowAsset.Visible = false
					WindowEnabled = false
				else
					WindowEnabled = true
					if WindowEnabled then
						WindowAsset.Visible = true
					end
				end
			end
		end
...
local function InitTab(ScreenAsset,WindowAsset,Window,Tab)
	...
	TabButtonAsset.Font = LibraryFont
...
local function InitSection(Parent,Section)
	...
	SectionAsset.Title.Font = LibraryFont
...
local function InitButton(Parent,ScreenAsset,Window,Button)
	...
	ButtonAsset.Title.Font = LibraryFont
...
local function InitToggle(Parent,ScreenAsset,Window,Toggle)
	...
	ToggleAsset.Title.Font = LibraryFont
    ToggleAsset.Tick.BackgroundColor3 = Toggle.Value and Window.Color or Color3.fromRGB(35,35,35)
...
local function InitSlider(Parent,ScreenAsset,Window,Slider)
	...
	SliderAsset.Title.Font = LibraryFont
...
local function InitTextbox(Parent,ScreenAsset,Window,Textbox)
	...
	TextboxAsset.Title.Font = LibraryFont
...
local function InitKeybind(Parent,ScreenAsset,Window,Keybind)
	...
	KeybindAsset.Title.Font = LibraryFont
...
local function InitDropdown(Parent,ScreenAsset,Window,Dropdown)
	...
	DropdownAsset.Title.Font = LibraryFont
	...
	for Index, Option in pairs(Dropdown.List) do
		...
		OptionAsset.Title.Font = LibraryFont
...
function Bracket:Window(Window)
	...
	Window.Keybind = Enum.KeyCode.RightBracket
